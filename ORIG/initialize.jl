...
struct paras
    ...
    relax_o2::Float64                  # Switch to use WOA-18 for O2 relaxation
    t_o2relax::Float64                 # Timescale of O2 relaxation
    o2_deep::Float64                   # Deep O2 (for alternate relaxation)
    o2_obs::Array{Float64,2}           # WOA-18 O2 profile (0 - 2000)
    relax_no3::Float64                 # Switch to use WOA-18 for NO3 relaxation
    t_no3relax::Float64                # Timescale of NO3 relaxation
    no3_obs::Array{Float64,2}          # WOA-18 NO3 profile (0 - 2000)
end
...
paras1 = paras(tt, nrec, H, dz, np, nb, nhets, nz, nn, nd, pIC, bIC, zIC, nIC, dIC, oIC,
            ...
            K_I, ngrid, GrM,
            e_o, koverh, o2sat, relax_o2, t_o2relax, o2_deep,o2_obs,relax_no3,
            t_no3relax,no3_obs)
